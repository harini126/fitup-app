...
function updateTotalTime() {
  ...
  document.getElementById('totalTime').innerText = "Total Time Spent: " + formattedTotalTime + " | Current Date: " + currentDate;

  // Store total time spent for today in localStorage
  storeTotalTime(currentDate, totalTimeSpent);
}

// (D) STORE TOTAL TIME IN LOCAL STORAGE FOR EACH DAY
...
// (E) RETRIEVE TOTAL TIME FROM LOCAL STORAGE FOR THE GIVEN DATE
...
// (F) RESET TOTAL TIME
// (A) VARIABLES
var lastCheckedDate = null; // Variable to store the last checked date

// (F) RESET TOTAL TIME ONLY IF DATE HAS CHANGED
function resetTotalTime() {
  ...
  if (currentDate !== lastCheckedDate) {
    // If the date has changed, reset the total time for the new date
    var storedData = localStorage.getItem('totalTimeData');

    // If data exists for today, remove it
    if (storedData !== null) {
      var data = JSON.parse(storedData);
      delete data[currentDate]; // Remove data for today
      localStorage.setItem('totalTimeData', JSON.stringify(data));
    }
